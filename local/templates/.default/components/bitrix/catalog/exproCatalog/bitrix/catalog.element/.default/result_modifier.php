...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
        $arResult['ALL_PRICE'][] = $arOffer["CATALOG_PRICE_1"];
...
//проверка на существование в массиве большеё цены
foreach($arResult['ALL_PRICE'] as $keyOffer => $arPrice){
    if($arPrice > $arResult['MIN_PRICE']["VALUE"]){
        $arResult["MAX_PRICE"] = $arPrice;
    }
}
...
//собираем цвета из инфоблока
$arResult["COLORS"] = array();
$el = CIBlockElement::GetList(array("SORT"=>"ASC"),array("IBLOCK_ID"=>7),false,false,array("ID","NAME","PREVIEW_PICTURE"));
while($arEl = $el->Fetch()) {
   $arResult["COLORS"][$arEl["ID"]] = $arEl;
}
