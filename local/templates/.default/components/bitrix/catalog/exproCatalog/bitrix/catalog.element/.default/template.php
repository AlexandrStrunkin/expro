...
<div style="display: none;" class="priceBlock">
    <div class="icons-product" >

        <div class="icon icon1"><span class="i"></span><?if($arResult['OFFERS'] and $arResult["MAX_PRICE"]){ echo 'от'; }?>  <?=$arResult['MIN_PRICE']["VALUE"]?> <img src="/img/rub2.png" class="rub" alt=""/><img src="/img/rub3.png" class="rub-hover" alt=""/></div>
        <?if ($arResult["PROPERTIES"]["SALE_LEADER"]["VALUE"]){?>
            <div class="icon icon2"><span class="i"></span>Лидер продаж</div>
            <?}?>
        <a href="#get-opp-price" class="icon icon3 login-popup-link" onclick="setOptProduct(<?=$arResult["ID"]?>,'<?=trim($arResult["NAME"])?>')"><span class="i"></span>Чтобы узнать оптовую цену, обратитесь к менеджеру</a>
        <a href="#" class="icon icon3"><span class="i"></span>Розничная цена при прямой покупке на условиях франко-склад поставщика</a>
    </div>
</div>
...
<script type="">
    $(function(){
        $(".catalog_top").append($(".priceBlock").html());
        $(".item_info__parametrs_more > div").jScrollPane({autoReinitialise: true});
...
<div class="catalog_wrapper catalog_wrapper_seria catalog_wrapper_tovar" >
    ...
    <div class="catalog-seria" id="<? echo $arItemIDs['ID']; ?>">
        ...
        ?>
        <div class="preview">
            <img src="<?=$firstPreviewBig["src"]?>" alt="<?=$arResult["DETAIL_PICTURE"]["ALT"]?>" />
        </div>
        ...
        <?
            ...
            foreach ($arResult["OFFERS"] as $offer) {
                $arResult['MORE_PHOTO'][] = $offer["DETAIL_PICTURE"];
            }
        ?>
        ...
        <div class="area1">
            <div class="img-container">
                ...
                <?if (is_array($arResult['MORE_PHOTO']) && count($arResult['MORE_PHOTO']) > 0) {?>
                    <?
                        foreach ($arResult['MORE_PHOTO'] as $pic) {?>
                        <?
                            $big = CFile::ResizeImageGet($pic["ID"], array("width"=>800,"height"=>700), BX_RESIZE_IMAGE_PROPORTIONAL ,false);
                            $small = CFile::ResizeImageGet($pic["ID"], array("width"=>110,"height"=>110), BX_RESIZE_IMAGE_PROPORTIONAL ,false);
                        ?>
                        <a href="<?=$big['src']?>"><img src="<?=$small['src']?>" alt=""/></a>
                        <?}?>
                    <a href="#" class="more">+<?=(count($arResult['MORE_PHOTO'])-3)?></a>
                    <?} else {/*?>
        ...
        <div class="area-container">
                        ...
                        <table class="characters">
                            <?
                                if ($arResult["SECTION"]["DEPTH_LEVEL"] == 3) {
                                    $arParent = CIBlockSection::GetById($arResult["SECTION"]["IBLOCK_SECTION_ID"])->Fetch();
                                }
                                if ($arParent["NAME"]) {
                                ?>
                                <tr>
                                ...
                                <?}?>
                            <?if ($arResult["PROPERTIES"]["DIMENSION"]["VALUE"]) {?>
                                <tr>
                                    <td class="data">Размеры:</td>
                                    <td class="value"><?=$arResult["PROPERTIES"]["DIMENSION"]["VALUE"]?></td>
                                </tr>
                                <?}?>
                            ...
                            <?if ($arResult["PROPERTIES"]["ARTICLE"]["VALUE"]) {?>
                                <tr>
                                    ...
                                    <td class="value"><?=$arResult["PROPERTIES"]["ARTICLE"]["VALUE"]?></td>
                                </tr>
                                <?}?>

                            <?if ($arResult["PROPERTIES"]["WEIGHT"]["VALUE"]) {?>
                                <tr>
                                    <td class="data">Вес:</td>
                                    <td class="value"><?=$arResult["PROPERTIES"]["WEIGHT"]["VALUE"]?></td>
                                </tr>
                                <?}?>
            ...
            <?foreach ($arResult["OFFERS"] as $offer){
                $arResult["PROPERTIES"]["COLORS"]["VALUE"][] = $offer["PROPERTIES"]["COLORS"]["VALUE"];
            }?>
            <?$arResult["PROPERTIES"]["COLORS"]["VALUE"] = array_unique($arResult["PROPERTIES"]["COLORS"]["VALUE"]);?>
            <?if (is_array($arResult["PROPERTIES"]["COLORS"]["VALUE"]) && count($arResult["PROPERTIES"]["COLORS"]["VALUE"]) > 0){?>
                <div class="area4">
                    <h3>Доступные цвета</h3>

                    <div class="img-container">
                        <?
                            foreach ($arResult["PROPERTIES"]["COLORS"]["VALUE"] as $ID) {
                                $color = $arResult["COLORS"][$ID];
                                $imgPath = CFile::GetPath($color["PREVIEW_PICTURE"]);
                            ?>
                            <a href="<?=$imgPath?>" class="fancy" title="<?=$color["NAME"]?>"><span><span><img src="<?=$imgPath?>" alt=""/></span></span><?=$color["NAME"]?></a>
                            <?}?>
                        <a href="javascript:void(0)" onmouseover="$(this).siblings('a.fancy').show(); $(this).fadeOut()" class="more"><span><span>+<?=count($arResult["PROPERTIES"]["COLORS"]["VALUE"]) - 4?></span></span>Цветовых сочетаний</a>
                    </div>
    ...
    <?if (is_array($arResult["OFFERS"]) && count($arResult["OFFERS"]) > 0) {?>
        <?
            ...
            usort($arResult["OFFERS"], "sortOffers");

        ?>
        <?foreach ($arResult["OFFERS"] as $n=>$offer){?>
            <div class="catalog_wrapper__item">
                ...
                <div class="catalog_wrapper__item__info">
                    <span class="item_info__title"><a href="#"><?=$offer["NAME"]?></a></span>
                    <div class="clearboth"></div>
                    <?//arshow($offer)?>
                    <? if (!empty($offer["DETAIL_TEXT"])) { ?>
                        <div class="detail_text">
                            <p><?=$offer["DETAIL_TEXT"]?></p>
                        </div>
                     <? } ?>
                    <div class="item_info__parametrs">
                        <? if (!empty($arResult["COLORS"][$offer["PROPERTIES"]["COLORS"]["VALUE"]]["NAME"])) { ?>
                            <div class="item_info__parametrs__item">
                                <span style="width: 50%;">Цвет:</span><span style="width: 50%;"><?=$arResult["COLORS"][$offer["PROPERTIES"]["COLORS"]["VALUE"]]["NAME"]?></span>
                            </div>
                            <? } ?>
                        <? if (!empty($offer["PROPERTIES"]["ARTICLE"]["VALUE"])) { ?>
                            <div class="item_info__parametrs__item">
                                <span>Артикул:</span><span><?=$offer["PROPERTIES"]["ARTICLE"]["VALUE"]?></span>
                            </div>
                            <? } ?>
                        <?if (is_array($offer["PROPERTIES"]["PARTS"]["VALUE"]) && count($offer["PROPERTIES"]["PARTS"]["VALUE"]) > 0) {?>
                            <div class="item_info__parametrs__item">
                                <span>Элементы изделий:</span><span class="plus">+<?=count($offer["PROPERTIES"]["PARTS"]["VALUE"])?></span>
                            </div>

                            <?$parts = array();
                                $part = CIBlockElement::GetList(array("SORT"=>"ASC"),array("IBLOCK_ID"=>12,"ID"=>$offer["PROPERTIES"]["PARTS"]["VALUE"]),false,false,array("ID","PROPERTY_COLOR","NAME"));
                                while($arPart = $part->Fetch()) {
                                    $parts[] = $arPart;
                                }
                            ?>
                            <?}?>
                    ...
                    <div class="item_info__parametrs_more">
                        <div>
                            <span class="item_info__parametrs_more-close"></span>
                            <span class="item_info__parametrs_more-title">Элементы изделий</span>

                            <div class="item_info__parametrs_more-colors">

                                <?foreach ($parts as $part) {?>

                                    <?$color = $arResult["COLORS"][$part["PROPERTY_COLOR_VALUE"]];
                                        $imgPath = CFile::GetPath($color["PREVIEW_PICTURE"]);
                                    ?>
                                    <a href="<?=$imgPath?>" class="fancy">
                                        <span><span>
                                                <img src="<?=$imgPath?>" alt=""/>
                                            </span>
                                        </span>
                            ...
                            <div class="item_info__parametrs_more-text">
                                <?foreach ($parts as $part) {
                                        $color = $arResult["COLORS"][$part["PROPERTY_COLOR_VALUE"]];
                                    ?>
